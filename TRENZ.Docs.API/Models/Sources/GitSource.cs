...
public class GitSource : AbstractFilesystemSource
{
    ...
    public override string Root => System.IO.Path.Combine(System.IO.Path.GetTempPath(), "TRENZ.Docs.API", Name);
    ...
    public override async Task UpdateAsync(CancellationToken cancellationToken = default)
    {
        ...
        Commands.Pull(
            repo,
            new("TRENZ.Docs.API", "trenz-docs@example.com", DateTimeOffset.Now),
            new()
            {
                MergeOptions = new()
                {
